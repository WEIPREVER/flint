...
export const StatementsUpdate = (props: RouteComponentProps<{ id: string }>) => {
  ...
  const saveEntity = values => {
    const entity = {
      ...statementsEntity,
      ...values,
      user: bankAccounts.find(it => it.id.toString() === values.user.toString()),
    };
        ...
        <Col md="8">
          {loading ? (
            ...
            <ValidatedForm defaultValues={defaultValues()} onSubmit={saveEntity}>
              ...
              />
              <ValidatedField id="statements-user" name="user" data-cy="user" label={translate('flintApp.statements.user')} type="select">
                <option value="" key="0" />
                {bankAccounts
